...
Spawn a `content-blog-researcher` agent (or do inline research with WebSearch):
...
Invoke both quality agents in parallel before delivering:

**Agent 1: `content-blog-quality-checker`**
- `content`: the full article text
- `primary_keyword`: the target keyword
- `frontmatter`: the parsed frontmatter fields

**Agent 2: `ai-writing-detector`**
- `text`: the full article text
- `context`: "blog post"

Wait for both to complete. If either fails, proceed with the available output and note the gap.

#### Review Agent Outputs

From `content-blog-quality-checker` output, flag any checks with `status: "fail"` or `status: "warn"`:
- **Fails** → fix before delivering (structural issues that directly hurt SEO or AI citability)
- **Warns** → note in delivery summary as recommended improvements

From `ai-writing-detector` output, fix any flagged issues:
- **Em dashes** (`severity: "elevated"` or `"high"`) → replace with appropriate punctuation per the pattern table in `content-refine`
- **AI-tell phrases** → remove or rewrite each flagged instance
- **Filler intensifiers** (`severity: "elevated"` or `"high"`) → cut or replace
- **Sentence variance** (`low_variance: true`) → add variation: break a long sentence, combine two short ones
- **Structural uniformity** (3+ flags) → convert one bulleted list to prose, vary paragraph openers

Also verify manually:
- **Contractions**: Natural use of "it's", "we've", "don't", "isn't". Formal AI prose avoids contractions.
- **Rhetorical questions**: At least one every 200-300 words to break up declarative patterns.
- **YouTube videos**: 2-3 embeds with lazy loading, aria-labels, and noscript fallback (see `references/video-embeds.md`).
...
### Quality Scores
- Structural score: [X]/100 (from content-blog-quality-checker)
- Human score: [X]/100 (from ai-writing-detector)
- Structural issues fixed: [list any fails/warns that were addressed]
- AI patterns removed: [em dash count, phrase count, filler count — from ai-writing-detector]
