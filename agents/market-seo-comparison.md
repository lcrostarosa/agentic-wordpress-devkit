...
description: Compare SEO posture across multiple websites — keyword visibility, SERP position tracking, content footprint, technical health, authority signals, AI citation presence. Tier 2 analysis — returns structured JSON with scoring.
model: sonnet
---
...
- **site_analysis_results**: market-site-analyzer JSON output for each entity (meta tags, headings, schema, performance, etc.)
- **competitor_profiles**: market-competitor-profiler JSON output (content depth, reviews, social presence)
...
- Cap WebSearch usage: maximum 8 keyword queries + 1 `site:` query per entity + 3 AI citation checks
